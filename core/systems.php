...
class Car 
{
    ...
    var $name;
    var $number_rents;
    var $status;            // 1 : Available , 2 : Rented , 3 : Broken
...
class CarsManger extends AbstractManger
{
    ...
    public function __construct($dbconnection)
    {
        ...
        $this->add_query              = "INSERT INTO ". DATABASE_NAME .".`cars` ( `name`, `status`, `number_rents`, `list_clients`, `image_path`) VALUES ( ?, ?, ?, ?, ?);";
        $this->delete_id_query        = "DELETE FROM ". DATABASE_NAME .".`cars` WHERE `id` ";
        $this->select_id_query        = "SELECT * FROM ". DATABASE_NAME .".`cars` WHERE `id` = ?";
        $this->select_range_query     = "SELECT * FROM ". DATABASE_NAME .".`cars` ORDER BY `number_rents` DESC LIMIT ";
        $this->update_id_query        = "UPDATE ". DATABASE_NAME .".`cars` SET ";
        $this->count_total_rows_query = "SELECT count(*) AS total FROM ". DATABASE_NAME .".`cars`";
    }
...
class RentsManger extends AbstractManger
{
    public function __construct($dbconnection)
    {
        ...
        $this->add_query          = "INSERT INTO ". DATABASE_NAME .".`rents` ( `client_id`, `car_id`, `price`, `creation_date`, `deadline_date`, `status`, `author_id`) VALUES ( ?, ?, ?, ?, ?, ?, ?);";
    ...
    {
        $query = "SELECT `id`, `client_id`, `car_id` FROM ". DATABASE_NAME .".`rents` 
...
function refresh_mangers($obj , $new_connection)
{
    ...
    if($obj & CARS_MANGER_FLAG)
    {
        if(!isset($_SESSION['CARS_MANGER']))
            $_SESSION['CARS_MANGER'] = new CarsManger($new_connection);
        else 
           $_SESSION['CARS_MANGER']->refresh_db_connection($new_connection);
    }
