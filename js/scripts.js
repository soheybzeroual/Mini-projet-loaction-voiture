...
var en = {
     ...
     MSG_CARINFO_EMPTY : "Car's name is undefined !"
};
...
var fr = {
    ...
    MSG_CARINFO_EMPTY : "Le nom du voiture n'est pas défini!"
};
...
function car_edit_form_submit(edit_form)
{
    ...
    for(var i = 0; i < checkboxes.length ; i++)
    {
        if(checkboxes[i].checked )
        {
            if(counter == 0)        // Pick the first one and wish the user didn't check more then 1 
            {
                ...
                obj.car_img_name = checkboxes[i].parentNode.parentNode.cells[6].childNodes[0].alt;
            }
            counter++;
        }
    }
    ...
    var car_name = edit_form['car_name'].value;

    if ( check_empty(car_name) && edit_form['car_img'].files.length == 0 && edit_form['status'].value == 0) 
    {
    ...
    edit_form["car_img_name"].value = obj.car_img_name;
}
    ...
    for(var i = 0; i < checkboxes.length ; i++)
    {
        if(checkboxes[i].checked )
        {
            if(counter == 0)        // Pick the first one and wish the user didn't check more then 1 
            {
                ...
                car_id  = checkboxes[i].parentNode.parentNode.cells[3].id;
            }
            counter++;
        }
    }
            ...
            switch(post_type)
            {
                ...
                case 2 :
                    if(num_ids == 0)
                    {
                        ...
                        var new_car_id_element = document.createElement("input");

                        new_car_id_element.type = "hidden";
                        new_car_id_element.name = "cars_id";
                        new_car_id_element.value = checkboxes[i].parentNode.parentNode.cells[3].id;

                        delete_form.appendChild(new_cl_id_element);
                        delete_form.appendChild(new_car_id_element);
                    }
                    break;
            }   
...
function verify_rent_data(add_form)
{
    if(check_empty(add_form["price"].value) || add_form["client_id"].value == "0" 
       || add_form["car_id"].value == "0" || check_empty(add_form["deadline"].value) )
    {
...
function verify_car_data(add_form)
{
    if(check_empty(add_form["car_name"].value))
    {
        alert(CURRENT_LANG.MSG_CARINFO_EMPTY);
        event.preventDefault();
    }
}